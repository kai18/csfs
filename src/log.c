...
void log_msg(const char *format, ...)
{
    ...
    vfprintf(csfs_DATA->logfile, format, ap);
}
...
void log_fuse_context(struct fuse_context *context)
{
    ...
    log_struct(((struct csfs_state *)context->private_data), logfile, %08x, );
    log_struct(((struct csfs_state *)context->private_data), rootdir, %s, );
